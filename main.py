...
async def post_init(application: Application) -> None:
    ...
    try:
        await application.bot.set_my_commands(instruction_commands, scope=BotCommandScopeChat(chat_id=ADMIN_USER_ID))
        logger.info(f"Set instruction commands for admin {ADMIN_USER_ID}.")
    except Exception as e:
        logger.error(f"Could not set instruction commands for admin {ADMIN_USER_ID}: {e}")
...
INSTRUCTION_LINKS = {
    ...
    "windows": "https://getoutline.org/ru/get-started/windows",
    "mac": "https://getoutline.org/ru/get-started/macos",
    "linux": "https://getoutline.org/ru/get-started/linux",
}
INSTRUCTION_TEXTS = {
    "mac": (
        "Инструкция для MacOS:\n\n"
        "1️⃣ Нажми на ключ доступа (выше ⬆️ начинается с ss://)\n"
        "2️⃣ Скачай и установи приложение: [Outline для Mac OS]({link})\n"
        "3️⃣ Запусти программу и вставь ключ\n"
        "4️⃣ Нажми \"Подключиться\" и наслаждайся высокой скоростью и стабильностью 😉"
    ),
    "windows": (
        "Инструкция для Windows:\n\n"
        "1️⃣ Нажми на ключ доступа (выше ⬆️ начинается с ss://)\n"
        "2️⃣ Скачай и установи приложение: [Outline для Windows]({link})\n"
        "3️⃣ Запусти программу и вставь ключ\n"
        "4️⃣ Нажми \"Подключиться\" и наслаждайся высокой скоростью и стабильностью 😉"
    ),
    "ios": (
        "Инструкция для iOS:\n\n"
        "1️⃣ Скопируй ключ доступа (выше ⬆️ начинается с ss://)\n"
        "2️⃣ Скачай и установи приложение: [Outline для iOS]({link})\n"
        "3️⃣ Открой приложение и вставь ключ\n"
        "4️⃣ Нажми \"Подключиться\" и пользуйся VPN!"
    ),
    "android": (
        "Инструкция для Android:\n\n"
        "1️⃣ Скопируй ключ доступа (выше ⬆️ начинается с ss://)\n"
        "2️⃣ Скачай и установи приложение: [Outline для Android]({link})\n"
        "3️⃣ Открой приложение и вставь ключ\n"
        "4️⃣ Нажми \"Подключиться\" и пользуйся VPN!"
    ),
    "linux": (
        ...
        "1️⃣ Скопируй ключ доступа (выше ⬆️ начинается с ss://)\n"
        "2️⃣ Скачай и установи приложение: [Outline для Linux]({link})\n"
        "3️⃣ Открой приложение и вставь ключ\n"
        "4️⃣ Нажми \"Подключиться\" и пользуйся VPN!"
    ),
}
...
INSTRUCTION_CHOOSE_PLATFORM = 10001

# --- Instruction Command and Button Handlers ---
@rate_limit_command("instruction")
async def instruction_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    keyboard = [
        [InlineKeyboardButton(name, callback_data=f"instruction_platform_{platform}")]
        for platform, name in INSTRUCTION_PLATFORMS
    ]
    keyboard.append([InlineKeyboardButton("❌ Отмена", callback_data="instruction_cancel")])
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(
        "Пожалуйста, выберите вашу платформу:",
...
async def menu_instruction_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await context.bot.send_message(
        chat_id=query.message.chat_id,
        text="Пожалуйста, выберите вашу платформу:",
        reply_markup=reply_markup
    )
    return INSTRUCTION_CHOOSE_PLATFORM
...
async def instruction_platform_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data.startswith("instruction_platform_"):
        ...
        text_template = INSTRUCTION_TEXTS.get(platform)
        ...
        if text_template and link:
            ...
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Назад", callback_data="menu_instruction")]])
            )
            return ConversationHandler.END
        else:
            await query.edit_message_text("Инструкция для этой платформы пока недоступна.")
            return ConversationHandler.END
