#!/usr/bin/env python3
"""
Lightweight VLESS-only Telegram Bot for VPS deployment
This bot only handles VLESS VPN functionality, no Outline.
"""
...
async def vless_subscribe(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Reload Xray configuration to apply the new user
        logger.info("Reloading Xray configuration...")
        from vless_utils_hybrid import reload_xray_config
        reload_xray_config()
...
def main() -> None:
    ...
    # Start the bot
    logger.info("Starting VLESS-only bot...")
